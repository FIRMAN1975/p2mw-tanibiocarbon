...
import { Sprout, Store, Inbox, UploadCloud, Settings, MapPin, Trash2, Check, X, ZoomIn, Lock, AlertTriangle, Truck, FlaskConical, Image as ImageIcon } from "lucide-react";
...
function LocationPickerWithReverseGeocode({ pinLocation, setPinLocation, alamatText, setAlamatText }) {
  ...
  return (
    <div className="border border-slate-200 rounded-2xl p-5 bg-slate-50/50">
      <label className="block text-sm font-semibold text-slate-900 mb-2 flex items-center gap-1.5"><MapPin className="w-4 h-4 text-red-500" /> Titik Penjemputan Truk Kargo *</label>
...
export default function PetaniPage() {
  ...
  const handleAjukanVerifikasi = async () => { 
    if(!phone || !rekening || !alamatLahan) return toast.error("Lengkapi profil dahulu!");
    await updateDoc(doc(db, "users", user.uid), { verificationStatus: "pending" }); toast.success("Berhasil diajukan ke Admin!"); 
  };
  const handleSaveProfile = async (e) => { e.preventDefault(); await updateDoc(doc(db, "users", user.uid), { phone, rekening_bank: rekening, alamat_lahan: alamatLahan, lokasi: pinLocation }); toast.success("Profil tersimpan!"); };
  
  const handleKirimTruk = async (orderId) => {
    const nopol = prompt("Nomor Polisi Truk Armada (Misal: B 1234 XX):"); const supir = prompt("Nama Supir:");
    if (!nopol || !supir) return toast.error("Pengiriman dibatalkan. Data tidak lengkap.");
    await updateDoc(doc(db, "orders", orderId), { status: "SHIPPED", no_polisi: nopol.toUpperCase(), nama_supir: supir, shippedAt: new Date() }); toast.success("Truk Berangkat!");
  };
  ...
  const handleUploadKomoditas = async (e) => {
    ...
    if (croppedImages.length === 0) return toast.error("Upload minimal 1 foto!");
    const tId = toast.loading("Mengupload..."); setLoading(true);
    try {
      ...
      toast.success("Berhasil dipublikasi!", { id: tId }); setNamaKomoditas(""); setCroppedImages([]); setActiveTab("etalase");
    } catch(err) { toast.error("Gagal mengupload", {id: tId}) } finally { setLoading(false); }
  };

  const handleToggleStatus = async (id, stat) => { await updateDoc(doc(db, "products", id), { status: stat === "tersedia" ? "terjual" : "tersedia" }); };
  const handleDeleteProduct = async (id) => { if (window.confirm("Hapus komoditas ini?")) await deleteDoc(doc(db, "products", id)); };
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 flex flex-col">
      {/* SECONDARY NAVIGATION (SaaS Style) */}
      <div className="bg-white border-b border-slate-200 sticky top-16 z-30">
        <div className="max-w-6xl mx-auto px-6">
          <div className="flex gap-6 overflow-x-auto no-scrollbar">
            <TabButton id="etalase" icon={Store} label="Etalase Saya" count={products.length} />
            <TabButton id="pesanan_masuk" icon={Inbox} label="Pesanan Masuk" count={orders.length} />
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto p-6 mt-4">
        ...
        {!isVerified && (
          <div className="mb-8 bg-amber-50 border border-amber-200 p-5 rounded-2xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div className="flex items-center gap-3">
              ...
              <div><h4 className="font-bold text-slate-900">Akun Belum Terverifikasi</h4><p className="text-sm text-slate-600">Lengkapi profil dan ajukan verifikasi agar dapat berjualan di Marketplace.</p></div>
            </div>
        ...
        {activeTab === "etalase" && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
            <h2 className="text-2xl font-extrabold text-slate-900 tracking-tight">Etalase Komoditas</h2>
            {products.length === 0 ? (
              <div className="text-center py-20 bg-white border border-dashed border-slate-300 rounded-3xl"><Store className="w-12 h-12 text-slate-300 mx-auto mb-4" /><h3 className="text-lg font-bold text-slate-900">Belum ada komoditas</h3><p className="text-slate-500 mb-6">Mulai jual bahan mentah pertama Anda.</p>{isVerified && <button onClick={()=>setActiveTab("upload")} className="bg-slate-900 text-white px-6 py-2.5 rounded-xl font-bold">Upload Sekarang</button>}</div>
            ) : (
        ...
        {activeTab === "pesanan_masuk" && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
            <h2 className="text-2xl font-extrabold text-slate-900 tracking-tight">Pesanan Masuk</h2>
            {orders.length === 0 ? (
              <div className="text-center py-20 bg-white border border-dashed border-slate-300 rounded-3xl"><Inbox className="w-12 h-12 text-slate-300 mx-auto mb-4" /><h3 className="text-lg font-bold text-slate-900">Belum ada pesanan</h3><p className="text-slate-500">Pesanan dari pembeli akan muncul di sini.</p></div>
            ) : (
                ...
                {orders.map(item => (
                  <div key={item.id} className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between gap-6">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2"><span className="text-[10px] font-mono font-bold bg-slate-100 text-slate-500 px-2 py-1 rounded-md border border-slate-200">ID: {item.id.slice(0,8)}</span></div>
                      <h3 className="text-xl font-black text-slate-900">{item.productName} <span className="text-slate-500 font-medium text-base">({item.totalTon} Ton)</span></h3>
                      <p className="text-sm text-slate-600 mt-1">Pembeli: <span className="font-semibold text-slate-900">{item.buyerName}</span></p>
                    </div>
                    <div className="w-full md:w-64 flex flex-col justify-between items-start md:items-end">
                      <div className="text-left md:text-right mb-4 md:mb-0"><p className="text-[10px] text-slate-400 font-bold uppercase">Total Pendapatan</p><p className="text-xl font-black text-green-600">Rp {item.totalPrice?.toLocaleString()}</p></div>
                      <div className="w-full text-right">
                        {item.status === "PAID_ESCROW" && <button onClick={()=>handleKirimTruk(item.id)} className="w-full md:w-auto bg-slate-900 hover:bg-slate-800 text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-lg transition-all flex items-center justify-center gap-2"><Truck className="w-4 h-4"/> Input Nopol Truk</button>}
                        {item.status === "SHIPPED" && <span className="inline-flex items-center gap-1.5 text-xs font-bold text-blue-700 bg-blue-50 px-3 py-2 rounded-xl border border-blue-200"><Truck className="w-4 h-4"/> Truk Menuju Lokasi Pembeli</span>}
                        {item.status === "WAITING_PAYMENT_SIMULATION" && <span className="inline-flex text-xs font-semibold text-amber-700 bg-amber-50 px-3 py-2 rounded-xl border border-amber-200">⏳ Menunggu Pembayaran Escrow</span>}
              ...
              <form onSubmit={handleUploadKomoditas} className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div><label className="block text-sm font-semibold text-slate-900 mb-1.5 flex items-center gap-1.5"><FlaskConical className="w-4 h-4 text-amber-500"/> Kadar Air (%)</label><input type="text" placeholder="Misal: < 15%" value={kadarAir} onChange={e=>setKadarAir(e.target.value)} className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-slate-900 focus:bg-white outline-none"/></div>
                    <div><label className="block text-sm font-semibold text-slate-900 mb-1.5 flex items-center gap-1.5"><FlaskConical className="w-4 h-4 text-purple-500"/> Kadar Karbon (%)</label><input type="text" placeholder="Misal: > 45%" value={kadarKarbon} onChange={e=>setKadarKarbon(e.target.value)} className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-slate-900 focus:bg-white outline-none"/></div>
                  </div>
                  
                  <div className="border border-slate-200 rounded-2xl p-5 bg-slate-50/50">
                    <label className="block text-sm font-semibold text-slate-900 mb-3 flex items-center gap-1.5"><ImageIcon className="w-4 h-4 text-blue-500"/> Foto Komoditas (Wajib)</label>
                    <label className="inline-flex items-center justify-center gap-2 bg-white border border-slate-200 hover:border-slate-400 text-slate-700 font-semibold px-5 py-3 rounded-xl cursor-pointer transition-all shadow-sm w-full md:w-auto"><UploadCloud className="w-4 h-4"/> Pilih Foto<input type="file" accept="image/*" onChange={handleFileSelect} className="hidden" /></label>
                    {croppedImages.length > 0 && (<div className="mt-4 pt-4 border-t border-slate-200 grid grid-cols-2 sm:grid-cols-4 gap-3">{croppedImages.map((img, index) => (<div key={index} className="relative aspect-[4/3] rounded-xl overflow-hidden border border-slate-200 shadow-sm"><img src={img.previewUrl} className="w-full h-full object-cover" /><button type="button" onClick={() => { setCroppedImages(p => p.filter((_, i) => i !== index)); }} className="absolute top-1.5 right-1.5 bg-white/90 p-1.5 rounded-full text-slate-700 hover:text-red-500 hover:bg-white transition-colors"><X className="w-3 h-3"/></button></div>))}</div>)}
                  </div>

                  <div><label className="block text-sm font-semibold text-slate-900 mb-1.5">Deskripsi Lengkap</label><textarea rows="3" placeholder="Informasi tambahan terkait kebersihan, jenis truk yang bisa masuk..." value={deskripsi} onChange={e=>setDeskripsi(e.target.value)} className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-slate-900 focus:bg-white outline-none"></textarea></div>
                  ...
                  <button type="submit" disabled={loading} className="w-full bg-slate-900 text-white font-bold py-4 rounded-xl shadow-lg hover:bg-slate-800 transition-all disabled:opacity-50">Publikasikan ke Marketplace</button>
                </div>
              </form>
            )}
          </div>
          ...
          <div className="max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-2 duration-300">
            <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
              <div className="flex justify-between items-center mb-8 pb-6 border-b border-slate-100">
                <div><h2 className="text-2xl font-extrabold text-slate-900 tracking-tight">Profil & Verifikasi</h2><p className="text-sm text-slate-500 mt-1">Data rekening untuk pencairan Escrow.</p></div>
              ...
              <form onSubmit={handleSaveProfile} className="space-y-5">
                <div><label className="block text-sm font-semibold text-slate-900 mb-1.5">No. WhatsApp</label><input type="text" value={phone} onChange={e=>setPhone(e.target.value)} className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-slate-900 focus:bg-white outline-none"/></div>
                <div><label className="block text-sm font-semibold text-slate-900 mb-1.5">Alamat Lengkap</label><input type="text" value={alamatLahan} onChange={e=>setAlamatLahan(e.target.value)} className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-slate-900 focus:bg-white outline-none"/></div>
                <div><label className="block text-sm font-semibold text-slate-900 mb-1.5">Informasi Rekening Bank</label><input type="text" placeholder="BCA 12345 a.n Nama" value={rekening} onChange={e=>setRekening(e.target.value)} className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-slate-900 focus:bg-white outline-none"/></div>
                
                <div className="pt-4 flex gap-4">
                  <button type="submit" className="flex-1 bg-white border border-slate-200 hover:bg-slate-50 text-slate-900 font-bold py-3.5 rounded-xl transition-all shadow-sm">Simpan Data</button>
                  {!isVerified && userProfile?.verificationStatus !== "pending" && (
                    <button type="button" onClick={handleAjukanVerifikasi} className="flex-1 bg-slate-900 hover:bg-slate-800 text-white font-bold py-3.5 rounded-xl shadow-lg transition-all">Ajukan Verifikasi</button>
                  )}
                </div>
              </form>
            </div>
