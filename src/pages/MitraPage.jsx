...
import { collection, query, where, onSnapshot, doc, updateDoc, addDoc, deleteDoc, serverTimestamp, getDoc } from "firebase/firestore";
import { ref, uploadBytes, getDownloadURL } from "firebase/storage";
import { APIProvider, Map, Marker, useMapsLibrary, useMap } from "@vis.gl/react-google-maps";
import { Factory, ShoppingBag, Truck, AlertTriangle, PlusCircle, Layers, Settings, MapPin, Trash2, Check, X, ZoomIn, Lock, MessageCircle, Star, FlaskConical, Image as ImageIcon, UploadCloud, ArrowDownToLine, ArrowUpFromLine, Wallet, Calculator, Store, FileText, AlertOctagon } from "lucide-react";
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto p-6 mt-4">
        {!isVerified && (
          <div className="mb-8 bg-amber-50 border border-amber-200 p-5 rounded-2xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div className="flex items-center gap-3"><div className="bg-amber-100 p-2 rounded-xl"><AlertTriangle className="w-5 h-5 text-amber-600" /></div><div><h4 className="font-bold text-slate-900">Pabrik Belum Terverifikasi</h4><p className="text-sm text-slate-600">Lengkapi profil agar bisa berjualan.</p></div></div>
            <button onClick={() => setActiveTab("profil")} className="bg-amber-600 hover:bg-amber-700 text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-sm">Buka Profil</button>
          </div>
        )}

        {/* TAB 1: BELANJAAN SAYA (BUYER) */}
        {activeTab === "belanja" && (
                    ...
                    <div className="flex flex-col md:flex-row justify-between border-b border-slate-100 pb-4">
                      <div>
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="text-[10px] font-bold bg-blue-50 text-blue-700 px-2 py-1 rounded border border-blue-100 uppercase flex items-center gap-1">
                            {item.shippingMethod === "pickup" ? <Store className="w-3 h-3"/> : <Truck className="w-3 h-3"/>}
                            {item.shippingMethod === "pickup" ? "Anda Ambil Sendiri" : "Diantar Penjual"}
                          </span>
                        </div>
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-center mt-2 gap-3">
                      ...
                      <div className="w-full sm:w-auto flex flex-wrap justify-end gap-2">
                        {item.status === "AWAITING_SHIPPING_COST" && (
                          <><button onClick={()=>handleBatalkanPesanan(item)} className="w-full sm:w-auto bg-white border border-slate-200 hover:bg-slate-50 text-slate-600 px-4 py-2.5 rounded-xl text-sm font-bold">Batalkan Pesanan</button><span className="w-full sm:w-auto text-xs font-bold text-amber-600 bg-amber-50 px-4 py-2.5 rounded-xl border border-amber-200 text-center">{item.proposedShippingCost ? "⏳ Menunggu Penjual Membalas Nego" : "⏳ Menunggu Penjual Menghitung Ongkir"}</span></>
                        )}
                        {item.status === "WAITING_PAYMENT_SIMULATION" && (
                          <><button onClick={()=>handleBatalkanPesanan(item)} className="bg-white border border-slate-200 px-4 py-2.5 rounded-xl text-sm font-bold">Batalkan</button>
                          ...
                          <button onClick={()=>handleBayarEscrow(item)} className="flex-1 sm:flex-none bg-slate-900 text-white px-5 py-2.5 rounded-xl text-sm font-bold shadow-lg">Bayar Tagihan Escrow</button></>
                        )}
            ...
            {sellerOrders.length === 0 ? (
              ...
              <div className="space-y-4">
                {sellerOrders.map(item => (
                  <div key={item.id} className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between gap-6 hover:shadow-md transition-all">
          ...
          <div className="max-w-3xl mx-auto animate-in fade-in slide-in-from-bottom-2 duration-300">
            {!isVerified ? (
              ...
              <form onSubmit={handleUploadKomoditas} className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm space-y-6">
                ...
                <div className="space-y-4">
                  <div><label className="block text-sm font-semibold text-slate-900 mb-1.5">Nama Produk</label><input type="text" value={namaKomoditas} onChange={e=>setNamaKomoditas(e.target.value)} className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none" required/></div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div><label className="block text-sm font-semibold text-slate-900 mb-1.5">Kategori</label><select value={kategori} onChange={e=>setKategori(e.target.value)} className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none"><option value="Biochar / Arang">Biochar / Arang</option><option value="Karbon Aktif">Karbon Aktif</option><option value="Wood Pellet">Wood Pellet</option></select></div>
                  ...
                  <div className="border border-slate-200 rounded-2xl p-5 bg-slate-50/50">
                    ...
                    <label className="inline-flex items-center gap-2 bg-white border px-5 py-3 rounded-xl cursor-pointer shadow-sm"><UploadCloud className="w-4 h-4"/> Pilih Foto<input type="file" accept="image/*" onChange={handleFileSelect} className="hidden" /></label>
                  ...
                  <div><label className="block text-sm font-semibold text-slate-900 mb-1.5">Deskripsi</label><textarea rows="3" value={deskripsi} onChange={e=>setDeskripsi(e.target.value)} className="w-full p-3.5 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none"></textarea></div>
